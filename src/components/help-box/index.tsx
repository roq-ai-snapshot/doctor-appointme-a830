...
export const HelpBox: React.FC = () => {
  ...
  const customerRoles: string[] = ['Patient'];
  const tenantRoles: string[] = ['Clinic Owner', 'Patient', 'Admin'];
  const addOns: string[] = ['notifications'];
  const defaultNotifications: string[] = [
    'create clinic - sent to owner',
    'create appointment - sent to owner, admin, patient',
    'update appointment - sent to owner, admin, patient',
  ];
  const applicationName = `Doctor Appointment`;
      ...
      !Boolean(session?.roqUserId)
        ...
        : ([
            ...
            addOns.includes('notifications') && {
              ...
              content: (
                <>
                  <Text mb="4">Here’s your notificaitons feed. Notifications can be configured through console</Text>
                  {defaultNotifications.length > 0 && (
                    <>
                      <Text mb="4">
                        The following are notifications generated initally triggered after some events:
                      </Text>
                      <UnorderedList>
                        {defaultNotifications.map((e, i) => (
